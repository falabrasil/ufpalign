...
if [ $STAGE -le 0 ] ; then
  echo "[$0] calling setup environment script" | lolcat
  steps/setup_env.sh $MODEL_EGS_DIR $DUMMY_EGS_DIR || exit 1
fi

if [ $STAGE -le 1 ] ; then
  echo "[$0] calling lang ext script" | lolcat
  steps/lang_ext.sh $DUMMY_EGS_DIR ../../dict_fb.dict || exit 1
fi
...
if [ $STAGE -le 3 ] ; then
  echo "[$0] calling mfcc + cmvn" | lolcat
...
if [ $STAGE -le 8 ] ; then
  echo "[$0] calling ivector extraction + nnet3 alignment scripts" | lolcat
  am_tag="tdnn"
  mkdir -p $WORKDIR/{ctm,pts}/$am_tag
